...
// 注意：
// - 本 crate 的 `Error` 是 `openlark_core::error::CoreError` 的类型别名
// - `From<reqwest::Error> for CoreError` 已在 `openlark-core` 中实现
// - 因此这里不再重复实现（否则会触发 Rust orphan rules）
...
mod tests {
    ...
    fn test_error_convenience_functions() {
        ...
        assert!(<Error as ClientErrorExt>::is_network_error(&network_err));
        assert!(network_err.is_retryable());

        let auth_err = authentication_error("令牌无效");
        assert!(<Error as ClientErrorExt>::is_auth_error(&auth_err));
        assert!(!auth_err.is_retryable());

        let validation_err = validation_error("email", "邮箱格式不正确");
        assert!(<Error as ClientErrorExt>::is_validation_error(&validation_err));
    ...
    fn test_client_error_ext() {
        ...
        assert!(!<Error as ClientErrorExt>::is_network_error(&error));
        assert!(!<Error as ClientErrorExt>::is_auth_error(&error));
        assert!(!<Error as ClientErrorExt>::is_business_error(&error));
        assert!(error.is_retryable());

        let suggestion = <Error as ClientErrorExt>::suggestion(&error);
        assert!(!suggestion.is_empty());

        let steps = <Error as ClientErrorExt>::recovery_steps(&error);
    ...
    fn test_sdk_result_conversion() {
        ...
        assert!(<Error as ClientErrorExt>::is_network_error(&client_result.unwrap_err()));
    }
}