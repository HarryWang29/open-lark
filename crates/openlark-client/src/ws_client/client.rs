...
use openlark_core::{api::Response, cache::QuickCache, event::dispatcher::EventDispatcherHandler};
...
async fn get_conn_url(
    ...
    // 使用用户配置的 base_url（支持飞书/国际站），并拼接 WebSocket endpoint
    let base = config.base_url.trim_end_matches('/');
    let endpoint_path = END_POINT_URL.trim_start_matches('/');
    let endpoint_url = format!("{base}/{endpoint_path}");
