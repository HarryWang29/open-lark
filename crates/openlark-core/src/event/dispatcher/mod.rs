...
where
    F: Fn(&[u8]) -> anyhow::Result<()> + Send + Sync,
{
...
impl EventDispatcherHandlerBuilder {
    /// 注册一个事件处理器（使用闭包）
    ///
    /// 事件名格式与内部派发一致：`{schema}.{event_type}`
    /// - v2 事件：`p2.im.message.receive_v1`
    /// - v1 事件：`p1.<type>`
    pub fn register<F>(mut self, event: impl Into<String>, f: F) -> Self
    where
        F: Fn(&[u8]) -> anyhow::Result<()> + Send + Sync + 'static,
    {
        self.processor_map
            .insert(event.into(), Box::new(FnEventHandler(f)));
        self
    }
