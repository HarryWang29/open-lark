...
// 说明：
// 飞书/开放平台的标准 JSON 响应通常为：
// { "code": 0, "msg": "success", "request_id": "...", "data": { ... } }
//
// 这里我们让：
// - raw_response 负责承载顶层元信息（以及 raw 的 data: Option<Value>）
// - data 负责把 raw_response.data 解析成具体类型 T，方便业务读取
    ...
    T: DeserializeOwned,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let data = match &raw_response.data {
            Some(raw) => serde_json::from_value::<T>(raw.clone()).ok(),
            None => None,
        };
